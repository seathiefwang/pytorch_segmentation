...
class Res101Unet(BaseModel):
    def __init__(self, num_classes, in_channels=3, freeze_bn=False, **_):
        ...
        # self.final_conv = nn.Conv2d(64, num_classes, kernel_size=1)
        self.final_conv = nn.Sequential(
            nn.Conv2d(64+64+64+64, 64, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
...
class DenseUnet(BaseModel):
    def __init__(self, num_classes, in_channels=3, freeze_bn=False, **_):
        ...
        model = models.densenet161(True).features
        self.down1 = nn.Sequential(model.conv0, 
                                    model.norm0,
                                    model.relu0) # 96
        self.down2 = nn.Sequential(model.denseblock1,
                                    model.transition1) # 192
        self.down3 = nn.Sequential(model.denseblock2,
                                    model.transition2) # 384
        self.down4 = nn.Sequential(model.denseblock3,
                                    model.transition3) # 1056
        self.middle_conv = nn.Sequential(
            nn.MaxPool2d(2, 2),
            DecoderAtt(1056, 528, 128, spatial=True)
        )
        # self.middle_conv = nn.Sequential(
        #     model.denseblock4,# 2208
        #     model.norm5,
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(2208, 1104, kernel_size=1, bias=False),
        #     nn.AvgPool2d(2, 2),
        #     DecoderAtt(1104, 552, 128, spatial=True)
        # )
        self.up1 = DecoderAtt(128+1056, 592, 64, channel=True)
        self.up2 = DecoderAtt(64+384, 224, 64, channel=True)
        self.up3 = DecoderAtt(64+64+192, 160, 64, channel=True)
        self.up4 = DecoderAtt(64+64+64+96, 144, 64, channel=True)
        # self.final_conv = nn.Conv2d(64, num_classes, kernel_size=1)
        self.final_conv = nn.Sequential(
            nn.Conv2d(64+64+64+64, 64, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm2d(64),
